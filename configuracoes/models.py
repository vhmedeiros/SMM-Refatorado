...
class ConfiguracaoCliente(models.Model):
    id_cliente = models.OneToOneField(
        ErpCliente, on_delete=models.CASCADE, primary_key=True, verbose_name="Cliente",
    ...
    empresa_prestadora = models.ForeignKey(
        ErpEmpresa, on_delete=models.CASCADE, verbose_name="Empresa Prestadora",
    ...
    url_pagina_cliente = models.CharField(max_length=255, null=True, blank=True, verbose_name="URL da Página",
                                          db_column="url_pagina_cliente")
    status_pagina = models.CharField(max_length=1, choices=[("A", "Ativo"), ("I", "Inativo")],
                                     verbose_name="Status da Página", db_column="status_pagina", default="I")
    data_ativacao = models.DateTimeField(null=True, blank=True, verbose_name="Data de Ativação", db_column="data_ativacao", default=now)
    ...
    def save(self, *args, **kwargs):
        # Se a página foi ativada e não tem data de ativação, define a data atual
        if self.status_pagina == "A" and not self.data_ativacao:
            self.data_ativacao = now()
        elif self.status_pagina == "I":
            self.data_ativacao = None  # Se a página for desativada, zera a data
